...
@php
  ...
  $allIncome = App\Models\Income::where('income_status',1)
    ...
    ->whereDay('income_date','=',$current_Day)->get();
  ...
  $allExpense = App\Models\Expense::where('expense_status',1)
    ...
    ->whereDay('expense_date','=',$current_Day)->get();

  $totalIncome = App\Models\Income::where('income_status',1)
    ->whereYear('income_date','=',$current_Year)
    ->whereMonth('income_date','=',$current_Month)
    ->whereDay('income_date','=',$current_Day)
    ->sum('income_amount');

  $totalExpense = App\Models\Expense::where('expense_status',1)
    ->whereYear('expense_date','=',$current_Year)
    ->whereMonth('expense_date','=',$current_Month)
    ->whereDay('expense_date','=',$current_Day)
    ->sum('expense_amount');
...
<div class="row"> <div class="col-md-12"> <div class="card mb-3"> <div class="card-header"> <div class="row"> <div
  ...
  <i class="fab fa-gg-circle"></i>Today Income Expense Statement </div> <div class="col-md-4 card_button_part">
    <a href="{{route('all-income')}}" class="btn btn-sm btn-dark"><i class="fas fa-th"></i>All Income</a> <a
      ...
      <div class="row"> <div class="col-md-2"></div> <div class="col-md-8">
        ...
        </div> @endif @if(Session::has('error')) <div class="alert alert-danger alert_error " role="alert">
          <strong>Opps! </strong>{{Session::get('error')}} </div>
        @endif
        </div>
